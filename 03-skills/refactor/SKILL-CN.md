---
name: code-refactor
description: 基于 Martin Fowler 方法论的系统化代码重构技能。适用于用户请求重构代码、改进代码结构、减少技术债、清理旧代码、消除代码异味或提升可维护性时。这个技能采用分阶段、带研究与计划的安全增量实施方式，并强调每一步都要可验证、可回滚、可沟通。
---

# 代码重构技能

这是一个基于 Martin Fowler《Refactoring: Improving the Design of Existing Code》（第 2 版）的系统化代码重构方法。这个技能强调安全、渐进的修改，并由测试提供保障；目标不是“重写”代码，而是在不改变外部行为的前提下，让内部结构更清晰、更稳定、更容易演进。

如果用户没有明确给出重构目标，先帮助他们把目标从“感觉代码很乱”收敛成具体问题，比如：提取重复逻辑、降低复杂度、拆分职责、改善测试可写性，或者减少后续改动的风险。

> “重构是在不改变软件外部行为的前提下，改进其内部结构的过程。” — Martin Fowler

## 核心原则

1. **保持行为不变**：外部行为必须保持一致，不能因为重构引入功能变化
2. **小步前进**：每次只做很小、可测试的改动，降低回退成本
3. **测试驱动**：测试是安全网，也是判断是否可以继续的依据
4. **持续进行**：重构是长期过程，不是一次性任务，而是持续改善代码健康度
5. **协作确认**：每个阶段都需要用户确认，避免在不确定的方向上越走越远
...
有了测试，才能把重构拆成小步，并在每一步都确认行为没有被破坏。
...
完整目录见 [references/code-smells-CN.md](references/code-smells-CN.md)。
...
完整列表见 [references/refactoring-catalog-CN.md](references/refactoring-catalog-CN.md)。
...
使用 [templates/refactoring-plan-CN.md](templates/refactoring-plan-CN.md) 里的模板。
...
- [代码异味目录](references/code-smells-CN.md) - 代码异味完整清单
- [重构目录](references/refactoring-catalog-CN.md) - 重构手法
- [重构计划模板](templates/refactoring-plan-CN.md) - 计划模板

## 脚本

- `scripts/analyze-complexity.py` - 分析代码复杂度指标
- `scripts/detect-smells.py` - 自动检测代码异味
