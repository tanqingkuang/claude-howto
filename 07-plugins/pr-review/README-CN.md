...
这个插件最适合带有 GitHub 仓库、CI 测试和明确代码审查流程的项目。先用 `/review-pr` 跑完整审查，再按需单独执行 `/check-security` 或 `/check-tests`，会更方便定位具体问题。
