...
斜杠命令是在 Claude 交互式会话中用来控制行为的快捷方式，主要分为几类：
...
- **MCP 提示词**：来自 MCP server 的命令，例如 `/mcp__github__list_prs`

> **注意**：自定义斜杠命令已经合并进 Skills。`.claude/commands/` 仍然可用，但现在更推荐使用 `.claude/skills/`。两者都会创建 `/command-name` 形式的快捷命令。完整参考请见 [Skills 指南](../03-skills/README-CN.md)。
...
以下 Skills 随 Claude Code 一起提供，调用方式和斜杠命令一样：
...
- MCP 提示词可作为 `/mcp__<server>__<prompt>` 命令使用，见 [MCP 提示词作为命令](#mcp-prompts-as-commands)

## 自定义命令（现已归入 Skills）

自定义斜杠命令已经**合并到 Skills**。两种方式都可以通过 `/command-name` 调用：
...
**文件：** [optimize-CN.md](optimize-CN.md)
...
**文件：** [generate-api-docs-CN.md](generate-api-docs-CN.md)
...
**文件：** [push-all-CN.md](push-all-CN.md)
...
**文件：** [doc-refactor-CN.md](doc-refactor-CN.md)

### 7. `/setup-ci-cd` - CI/CD 流水线配置

实现 pre-commit hooks 和 GitHub Actions 质量保障流程。

**文件：** [setup-ci-cd-CN.md](setup-ci-cd-CN.md)

### 8. `/unit-test-expand` - 测试覆盖率扩展

针对未测试分支和边界情况，提升测试覆盖率。

**文件：** [unit-test-expand-CN.md](unit-test-expand-CN.md)
...
- **[Skills](../03-skills/README-CN.md)** - 完整的 Skills 参考
- **[Memory](../02-memory/README-CN.md)** - 带 `CLAUDE.md` 的持久上下文
- **[子代理](../04-subagents/README-CN.md)** - 委派式 AI agents
- **[Plugins](../07-plugins/README-CN.md)** - 打包好的命令集合
- **[Hooks](../06-hooks/README-CN.md)** - 事件驱动自动化
